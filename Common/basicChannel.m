...
num_bit_err2 = zeros(length(ebno_vec), 1);
num_pack_err = zeros(length(ebno_vec), 1);
num_pack_err2 = zeros(length(ebno_vec), 1);
...
for i_run = 1 : max_runs
    if  mod(i_run, max_runs/resolution) == 1
        ...
        disp(num2str([ebno_vec'  num_pack_err./num_runs  num_bit_err./num_runs/K num_bit_err2./num_runs/K]));
        disp(' ');
    end
...
%         num_bit_err2(i_ebno) = num_bit_err2(i_ebno) + biterr(chase2codeout, info);
        ...
        num_bit_err2(i_ebno) = num_bit_err2(i_ebno) + biterr(info_bch_decode2, info);
        if any(info_bch_decode2 ~= info)
            num_pack_err2(i_ebno) =  num_pack_err2(i_ebno) + 1;   
        end 
    end
end
toc
...
ber2 = num_bit_err2./num_runs/K;
ber_chase2 = num_bit_err2./num_runs/K;
per = num_pack_err./num_runs;
per2 = num_pack_err2./num_runs;
...
figure()
p = semilogy(ebno_vec, [per';ber';unionBoundComp';ber_theoretical_bpsk]);
grid on
p(1).Marker = 's';
...
l = legend('per','ber', 'Union Bound', 'ber theoretical bpsk');
